...
function validateScope(isValid, scope) {
  ...
  var regexpMode = config.scope.regexpMode || false;
  var scopes = scope ? scope.split(',') : [];

  function validateIndividualScope(item) {
    var NOT_ALLOWED_SCOPE_ERR_MSG = '"%s" is not an allowed scope ! Valid scope are: %s';

    if (allowedScopes[0].trim() === '*') {
      return;
    }
    if (!regexpMode && allowedScopes.indexOf(item) === -1) {
      error(NOT_ALLOWED_SCOPE_ERR_MSG, item, allowedScopes.join(', '));
      isValid = false;
    }
    if (regexpMode && !allowedScopes.some(regexpComparator)) {
      error(NOT_ALLOWED_SCOPE_ERR_MSG, item, allowedScopes.join(', '));
      isValid = false;
    }

    function regexpComparator(allowedScope) {
      return new RegExp(allowedScope).exec(item);
    }
  }
